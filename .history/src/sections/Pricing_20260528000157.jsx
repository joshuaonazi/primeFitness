    ...
    <section id="pricing" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-gray-900 mt-2">Simple <span className="text-green-700">Pricing</span></h2>
          <p className="text-gray-500 mt-4 max-w-xl mx-auto text-lg">No hidden fees. No long term contracts. Just results.</p>
        </motion.div>

        {/* Plans */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          {plans.map((plan, index) => (
            <motion.div key={plan.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: true }} className={`rounded-2xl p-8 ${plan.highlighted ? 'bg-green-700 text-white shadow-2xl scale-105' : 'bg-gray-50 text-gray-900 shadow-sm'}`}>
              ...
              <a href="#" className={`block text-center py-3 rounded-full font-bold transition-colors duration-200 ${plan.highlighted ? 'bg-white text-green-700 hover:bg-green-50' : 'bg-green-700 text-white hover:bg-green-800'}`}>
                Get Started
              </a>
